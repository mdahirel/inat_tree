...
#| output: true
inat_names <- get_inat(project_id="invertefest-april-2025")
```
...
d2 <- data.frame(
  node = c(
    ...
    length(my_tree$tip.label) + which(my_tree$node.label == "Myriapoda"),
    length(my_tree$tip.label) + which(my_tree$node.label == "Cnidaria")
  ),
  taxon = c(
    "Diptera", "Hymenoptera", "Coleoptera", "Lepidoptera",
    "Orthoptera", "Araneae", "Gastropoda", "Odonata", "Hemiptera", "Myriapoda","Cnidaria"
  ),
  image = c(
    ...
    get_uuid(name = "Actinia equina", n = 1)
  )
) |> 
...
p <- ggtree(my_tree, layout = "circular", size=tree_line_width) +
  ...
  theme(text=element_text(family = "Ubuntu", size = 12),
        ...
        plot.caption = element_textbox_simple(family = "Ubuntu",size=6, halign=1),
